/**
 * AegisLink — Mailbox root store (sealed-sender Fase 4, desktop)
 *
 * Parity with mobile/src/crypto/mailboxStore.ts. Owns the client side of blind
 * mailbox IDs (docs/SEALED-SENDER-ARCHITECTURE.md §3.4). Mirrors deliveryToken.ts:
 *  - our OWN 32-byte mailbox root: generated once, persisted via secureStorage,
 *    shared with contacts over E2EE (profile_update). Whoever holds it can derive
 *    every epoch's mailbox id for us, so it is a secret → secureStorage.
 *  - CONTACTS' roots: persisted per contact so we can compute the recipient's
 *    current mailbox id when addressing a message.
 *
 ...
 *
 * Persists via window.aegis.secureStorage (same channel as identity secrets and
 * the delivery token).
 */
 ...
 * a secret (whoever holds it can derive our mailbox for any epoch) → secureStorage.
 */
export async function getOwnMailboxRoot(): Promise<Uint8Array> {
  const existing = await secureStorage().get(OWN_ROOT_SLOT);
  if (existing) {
    const valid = isValidRootB64(existing);
    if (valid) return valid;
  }
  const fresh = nacl.randomBytes(ROOT_BYTES);
  await secureStorage().set(OWN_ROOT_SLOT, encodeBase64(fresh));
  return fresh;
}
...
export async function setContactMailboxRoot(aegisId: string, rootB64: string): Promise<void> {
  ...
  await secureStorage().set(peerSlot(aegisId), rootB64);
}
...
export async function getContactMailboxRoot(aegisId: string): Promise<Uint8Array | null> {
  const v = await secureStorage().get(peerSlot(aegisId));
